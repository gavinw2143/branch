...
import { GeneratingSetContext, IdContext, IdSetContext, LeftRightContext, LeftRightSetContext, ResponseContext, ResponseDispatchContext } from '@/app/lib/context';
...
export default function Followup({ selectionText, handleFollowup, lr, position }: FollowupTypes) {
    ...
    async function handleSubmit(e) {
        ...
        if (thisId === 1) {
            ...
            dispatchResponses({type: 'add', query: query, id: thisId, parent: parentId, response: ''});
        }

        setGenerating(true);

        const history = responses.find(r => r.id === parentId).history;
